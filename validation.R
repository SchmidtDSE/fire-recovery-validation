...
compare_metrics = function(df_dnbr, df_rdnbr) {
...
results_rdnbr = st_drop_geometry(df_rdnbr) %>%
  filter(fire_days == 30) %>%
  select(fire_name, fire_days, metric, calfire_mean, calfire_var)

results_rdnbr_parks = results_rdnbr %>%
  left_join(select(calfire, c("fire_name", "UNIT_ID")))

results_parks = bind_rows(results_dnbr_parks, results_rdnbr_parks) %>%
...
results = bind_rows(results_dnbr, results_rdnbr) %>%
  rename('Mean burn severity' = calfire_mean, 'Variance of burn severity' = calfire_var) %>%
  pivot_longer(cols = c(`Mean burn severity`, `Variance of burn severity`), names_to = 'name', values_to = 'value') %>%
  mutate(UNIT_ID = 'All Parks')
...
label_data = results %>%
  ...
  mutate(UNIT_ID = "All Parks",
         label_text = ifelse(metric == 'dnbr', 'dNBR', 'rdNBR'))

(p = ggplot() + theme_bw() +
    ggtitle('Comparison of dNBR and rdNBR') +
    ...
    scale_x_discrete(labels = c('All Parks', 'Channel Islands', 'Joshua Tree', 'Seqouia', 'Mojave', 'Santa Monica'), name = "") +
    scale_y_continuous('Burn severity', expand = c(0,.1)) +
    scale_fill_scico_d(palette = 'lajolla', guide = 'none', begin = .3 ) +
    scale_color_manual(values = c('dnbr' = 'grey0', 'rdnbr' = 'grey1'), name = 'Metric') +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
        text = element_text(size = 14),
        legend.position = 'None'))
...
results_rdnbr = st_drop_geometry(df_dnbr) %>%
  select(fire_name, fire_days, metric, calfire_mean, calfire_var) %>%
  group_by(fire_name) %>%
  ...
  p = ggplot() + theme_bw() +
    facet_wrap(~name, ncol = 1, scales = 'free') +
    geom_jitter(data = results_rdnbr, aes(x = fire_days, y = value, color = UNIT_ID, shape = UNIT_ID), width = .5) +
    geom_smooth(data = results_rdnbr, aes(x = fire_days, y = value), method = 'loess', color = 'black') +
