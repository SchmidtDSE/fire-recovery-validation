...
def plot_fire(fire_name, fire_days, fire_polygon, fires, date_mode='alarm', sensor='sentinel-2'):
    ...
    # dNBR
    im1 = axes[0].imshow(dnbr_reproj.values[0], cmap='RdYlGn_r', vmin=vmin, vmax=vmax,
    ...
    axes[0].set_title(f'dNBR ({fire_name} after {fire_days} days, {sensor})')
    axes[0].set_xlabel('Longitude')
    axes[0].set_ylabel('Latitude')
    plt.colorbar(im1, ax=axes[0])

    # RBR
    im2 = axes[1].imshow(rbr_reproj.values[0], cmap='RdYlGn_r', vmin=vmin, vmax=vmax,
    ...
    axes[1].set_title(f'RBR ({fire_name} after {fire_days} days, {sensor})')
